...
- [Step 3](#step-3-make-a-payment): Set Payment Extension Request custom field to commercetools payment to make payment via extension..
- [Step 4](#step-4-update-payment-status): Submit a payment request by setting `makePaymentRequest` payment custom field with the payment data returned by the Adyen web component.
- [Test and go live](#test-and-go-live): Set `submitAdditionalPaymentDetailsRequest ` custom field to commercetools payment to submit additional payment details.
- [Additional information](#additional-information): Add an optional `Charge` transaction to commercetools payment in order to manually capture the payment.
