...
describe('handler::notification::notification.handler', () => {
    ...
    test('not POST request', () => request(server)
        ...
        .expect(400)
        .then((response) => {
            expect(response.text).toEqual('Received non-POST request: GET. The request ' +
    ...
    test('not found reference', () => {
        ...
        return request(server)
            ...
            .expect(200)
            .then((response) => {
                expect(response.text).toEqual(JSON.stringify({status: "Failure", message: "Reference not found"}))
            })
    })
    ...
    test('transaction success (paydock-paid)', () => {
        ...
        return request(server)
            ...
            .then((response) => {
                ...
                expect(response.text).toEqual(JSON.stringify({status: "Success"}))
            })
    })
